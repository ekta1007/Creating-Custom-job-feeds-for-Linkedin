...
def tf(word, doc,docs):
    return (freq(word, doc) / float(word_count(docs,doc)))
...
def tf_idf(word, doc, list_of_docs):
    return (tf(word, doc,docs) * idf(word, list_of_docs))


for doc_no in doc_list:
    docs[doc_no] = {'freq': {}, 'tf': {}, 'idf': {},'tf-idf': {}, 'tokens': []}
    tokens = tokenizer.tokenize(doc_no)
    docs[doc_no]['tokens']=tokens
    for token in tokens:
        docs[doc_no]['freq'][token]=freq(token, doc_no)
        docs[doc_no]['tf'][token]=tf(token, doc_no,docs)
        docs[doc_no]['idf'][token]=idf(token, doc_list)
        docs[doc_no]['tf-idf'][token]=tf_idf(token, doc_no, doc_list)
